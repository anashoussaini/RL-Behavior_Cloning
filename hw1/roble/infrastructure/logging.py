...
#             logger.set_step(step=settings["round"])
    ...
    def dump_tabular(self, *args, **kwargs):
        if len(self._tabular) > 0:
            # Convert tabular data to a dictionary for easier processing
            tabular_dict = dict(self._tabular)

            # Prepare the formatted string for the log file
            log_str = ""
            for key, val in tabular_dict.items():
                log_str += f"{key}: {val}\n"

            # Write to log files
            for log_fd in list(self._tabular_fds.values()):
                if not isinstance(log_fd, str):
                    print(log_fd)  # Checking if log_fd is a file descriptor
                    log_fd.write(log_str)
                    log_fd.flush()
                else:
                    # Handle the case where log_fd might be a file path
                    with open(log_fd, 'a') as file:
                        file.write(log_str)
                        file.flush()

            # Clear the tabular list for the next logging iteration
            del self._tabular[:]
    ...
    def log_video_file(self, video_frames, name, step, fps=24):
        ...
        for video_idx in range(len(video_frames)):
            clip = ImageSequenceClip(list(video_frames[video_idx]), fps=24)
            clip_name = f'{name}_step_{step}_video{video_idx+1}.mp4'
            print(f"FPS before writing video file: {fps}")
            clip.write_videofile(str(self._log_video_path / clip_name), fps=24, verbose=False)

    def log_paths_as_videos(self, paths, step, max_videos_to_save=2, fps=24, video_title='video'):
